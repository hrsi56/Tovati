# ארכיטקטורת האפליקציה — גרסה 1.5.0
...
| `feature/today` | איחוד נתוני היום והצגת תחזית, מהימנות ופעולה הבאה |
| `feature/entry` | עריכת מדידת BBT ותצפית יומית, לרבות נוזל צוואר הרחם, מצב רוח, חשק, מגע, תסמינים ו־LH |
| `feature/chart` | סדרת BBT ושכבות נפרדות לטווח התעברות, תחזית פרוספקטיבית והערכה רטרוספקטיבית |
...
| `daily_observations` | רשומה ייחודית ליום עבור דימום, מראה ותחושת נוזל צוואר הרחם, סימון תצפית שקשה לפרש, תוצאת LH ומטא־נתונים אופציונליים, כאב, מצב רוח והערה, חשק/אוננות, מגע מיני ומי יזמה, תסמינים, כדורים להקלה והתחלת מחזור |
...
`CycleAnalysisEngine` מקבל snapshot בלתי תלוי ב־Android הכולל מחזור נוכחי, מדידות עד תאריך הניתוח, תצפיות, מחזורים היסטוריים ואתר ניתוח קבוע. גרסתו `bbt-fusion-2.2.0`.
...
- `PersonalPriorCalculator` בונה prior רחב ומותאם בהדרגה כבר מהמחזור המלא הראשון; אין gate קשיח של שלושה מחזורים.
...
- `MucusSignalAnalyzer` משלב מראה ותחושה, מתעלם מתצפית שסומנה מוסתרת ומזהה עלייה ושיא שהושלם.
...
- `CycleAnalysisEngine` מאחד את האותות, מרחיב תוצאה בסתירה ומחזיר בנפרד טווח ביוץ פרוספקטיבי, חלון התעברות וטווח רטרוספקטיבי.
...
## גיבוי schema v4, שחזור סכמות קודמות ו־CSV

הגישה לקבצים משתמשת ב־Storage Access Framework באמצעות `CreateDocument` ו־`OpenDocument`, ולכן אינה דורשת הרשאת אחסון רחבה.

- גיבוי חדש נוצר כ־`BackupPayload` schema v4 עם `appVersion` שנלקח אוטומטית מגרסת הבנייה הנוכחית, כולל מחזורים, מדידות, תצפיות והגדרות, ואז מוצפן לפני כתיבה למסמך.
- JSON מקודד עם ברירות מחדל ו־null מפורשים; מפתחות לא מוכרים אינם מתקבלים בשקט.
- השחזור תומך בסכמות 1–4. בשחזור סכמות ישנות שדות התצפית החדשים מקבלים את ברירות המחדל של המודל, ו־`analysisSite` מושלם מהמדידה הנבחרת הראשונה במחזור כאשר אפשר.
...
`BackupPayloadCompatibilityTest` מכסה פענוח payload v1 והשלמת `analysisSite`; `BackupManagerInstrumentedTest` מכסה round-trip מוצפן של schema v4 וכל עמודות CSV, ו־`ShareableBackupInstrumentedTest` מכסה את קובץ השיתוף המוצפן ואת ה־content URI.
...
הגרסאות מרוכזות ב־`gradle/libs.versions.toml`. גרסה 1.5.0 משתמשת ב־`versionCode=7`, נבנית עם Java 17, `compileSdk/targetSdk 36` ו־`minSdk 26`. build מסוג release מפעיל R8 וכיווץ resources, וחתימה נטענת רק מ־`keystore.properties` מקומי.
