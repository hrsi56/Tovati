# ארכיטקטורת האפליקציה — גרסה 1.5.1
...
הגרסאות מרוכזות ב־`gradle/libs.versions.toml`. גרסה 1.5.1 משתמשת ב־`versionCode=8`, נבנית עם Java 17, `compileSdk/targetSdk 36` ו־`minSdk 26`. build מסוג release מפעיל R8 וכיווץ resources, וחתימה נטענת רק מ־`keystore.properties` מקומי.
