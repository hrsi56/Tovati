...
`CycleAnalysisEngine` מקבל snapshot בלתי תלוי ב־Android הכולל מחזור נוכחי, מדידות עד תאריך הניתוח, תצפיות, מחזורים היסטוריים, אתר ניתוח תאימות ואורך מחזור אופציונלי שמסרה המשתמשת ב־onboarding. גרסתו `bbt-fusion-3.0.0`.
...
- `LhSignalAnalyzer` מקבץ חיוביים לאפיזודות המעוגנות בחיובי הראשון ומבדיל תוצאה גבולית. LH הוא תוספת אופציונלית; האות פעיל עד `+2`, חיובי ממושך אינו מזיז את העוגן, ויום שלא נבדק אינו נחשב שלילי.
- `MucusSignalAnalyzer` משלב מרקם ותחושה, מתעלם מתצפית שסומנה מוסתרת ומזהה עלייה ושיא שהושלם.
- שכבת ה־repository שומרת את `analysisSite` לצורכי תאימות לנתונים ישנים; `ThermalShiftDetector` משתמש בדרך המדידה העקבית, מחריג מדידות שאינן אמינות לאישור, סורק את כל המועמדים ובודק התמדה או הפרכה של דפוס. `ThermalTrendDetector` יכול לזהות תמיכה מוגבלת משלוש מדידות גבוהות שנצפו בפועל גם כאשר יום ביניהן חסר, בלי להשלים את המדידה ובלי להכריז על אישור.
